...
if (file_exists("deneme") == true) {
	touch("deneme/deneme.txt"); //dosya oluşturur
	touch("deneme/selam.txt"); //touch: dosya oluşturur
...
//w+: dosyayı açar ve dosyanın içeriğini siler
//r+: dosyayı açar ve dosyanın içeriğini silmez
//a+: dosyayı açar ve dosyanın içeriğini silmez, dosyanın sonuna ekler
//x+: dosyayı açar ve dosyanın içeriğini silmez, dosya yoksa hata verir
//c+: dosyayı açar ve dosyanın içeriğini silmez, dosya yoksa oluşturur 
...
echo "<br/>//Deneme<br/>";
fseek($dosyamiz, 0); //fseek: dosya okuma konumunu belirler
echo fread($dosyamiz, 10) . "<br/>"; //fread: dosyadan belirtilen byte kadar okur
fclose($dosyamiz);
...
while ($j < $max) {
	...
	if ($j + 1 != $max)
		fputs($yenidosya, PHP_EOL); //PHP_EOL: satır sonu karakteri ekler
	$j++;
}
fclose($yenidosya);
...
while (!feof($toplama)) {
	$icerik += (int) fgets($toplama); //fgets: dosyadan bir satır okur 
}
fclose($toplama);
...
fpassthru($topluokuma); //fpassthru: dosyanın tamamını okur
fclose($topluokuma);
...
$liste = glob("*.php"); //glob: belirtilen kalıba uyan dosya ve dizinleri döndürür
echo "<pre>";
print_r($liste);
echo "</pre>";
echo "<br/><br/>";
